...
- Повний набір тестів після першої реалізації: **18/18** (11 наявних + 7 ERP).
- Сценарій 150 виробів перевірений у транзакційних тестах, включно з rollback прийомки при дублікованому номері.
- Playwright пройшов: навігація адміністратора, деталі замовлення, форма прийомки, ізоляція майстра, pause/resume/complete на телефоні, зміни, ширина 360 px, відсутність JS/CSP помилок.
- Після цього додано серверну пагінацію особистої черги, зміну метаданих замовлення, закриття зміни керівником, детальний редактор шаблонів та темну/світлу палітру. Нові theme/continuation-зміни потребують фінального повторного браузерного прогону та оновлення скриншотів.
- `npm run check` після пагінації та управлінських дій пройшов; ERP-тести 7/7 пройшли. Додати поведінкові тести для пагінації й керівного закриття зміни перед остаточним release.
...
1. Перезапустити локальний server з актуальним кодом, прогнати поведінкові тести й browser QA темної/світлої теми, persistence та mobile.
2. Перевірити міграцію на копії поточної схеми без втрати наявних рядків; перевірити отримання `403` ERP користувачем без ролі.
3. Пройти точні `npm ci`, `npm run check`, `npm test`, `npm audit --omit=dev`. Зробити обмежений performance smoke, не навантажувати production.
4. Оновити `README`, `SECURITY`, `DEPLOYMENT`; відобразити відомі межі першої версії. Усі незавершені модулі з `ERP.md` залишаються планом, а не нібито реалізованими функціями.
5. `git fetch origin`, звірити main та production code з базою. Секрети не виводити. Створити і перевірити SQLite `.backup`.
