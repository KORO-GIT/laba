...
`codex/erp-material-planning` від clean `origin/main=eed5419`. Реалізуються каталог/норми на шаблон і замовлення, розрахунок всіх відкритих потреб, внутрішні попередження, заявки з частковим надходженням, підтвердження фактичної витрати. Повністю прочитати `ERP_MATERIALS.md`; production до перевіреного deployment залишається `c947dcb/0.25.0`. Функціональні 22 ERP-тести й JS syntax пройшли; повні перевірки/локальний browser/VPS stage/deploy ще попереду. Код і docs push як checkpoint, не видавати його за готовий production. Особливо важливо зберігати `quantity_scale` та незмінні legacy рухи.
