...
- Поточна версія застосунку: `0.23.2`.
- Останній підтверджений production-код: commit `35bb176ef83a59bac9a7be26e590784d4c863a89`; інтеграційна частина SignalSynch — commit `8f81ae22bb2e428de89b608d26e6adf7aebe63f3`. `laba-portal.service`, `koro-signal-sheets-sync.service` і Caddy активні, `/healthz` відповідає, `PRAGMA quick_check` повертає `ok`, а production-тести LABA проходять `9/9`. Caddyfile під час deployment не змінювався; його контрольна сума залишилася незмінною. Стартова й наступна інтервальна синхронізації передали по 221 запису без помилок і незавершених outbox-дій.
