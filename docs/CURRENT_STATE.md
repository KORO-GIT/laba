...
- Поточна версія застосунку: `0.24.0`.
- Останній підтверджений production-код: **`e430efd1366eb663c2f79ab92d66a5924c027855`**, перший ERP-контур. Розгорнуто 2026-09-07 близько 20:22–20:24 UTC; усі 88 файлів Git archive на VPS звірено за SHA-256 без розбіжностей. Гілку `codex/laba-erp` fast-forward об'єднано та push у `main`. Наступні documentation-only commits не означають нового deployment коду.
- Перед ERP deployment створено SQLite backup **`/opt/laba/backups/portal-20260907-2022-before-0.24.0.db`**. Попередній код `0.23.3` збережено в **`/opt/laba-previous-20260907T2022Z-0.23.3`**. `.env`, актуальні `data/` і `backups/` скопійовано після зупинки лише LABA, перевірено їх тотожність до старту. Після старту ERP додала лише свою схему; старі таблиці не перебудовувалися.
...
- Виробництво ERP: `https://laba.zpseapil.club/erp`; майстрам — `/erp#my`.
...
## ERP 0.24.0: випуск і наступні кроки
