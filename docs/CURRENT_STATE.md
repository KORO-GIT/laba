...
Raspberry Pi має зарезервовану адресу `192.168.0.63`. Vendor WayVNC слухає TLS/PAM `192.168.0.63:5900`; у локальній мережі до цієї адреси підключається звичайний VNC-клієнт. Окремий `laba-wayvnc-web.service` без другого GPU encoder слухає `192.168.0.63:5901`, використовує PAM/VeNCrypt Plain і systemd IP policy, яка приймає лише VPS та власні адреси Pi. Служба використовує перевірену patched-збірку Raspberry Pi WayVNC `0.9.1-1+rpt5`: для headless-виходу labwc зі станом живлення `UNKNOWN` capture починається одразу, а `ext_image_copy_capture` залишається ввімкненим. `laba-wayvnc-attach.service` прив’язує endpoint до активного Wayland compositor і відновлює прив’язку після рестарту сесії. Plain потрібен через обмеження noVNC і знаходиться лише всередині HTTPS/WSS та Tailscale/WireGuard.
