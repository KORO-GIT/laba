...
- Поточна версія застосунку: `0.21.0`.
- Останній підтверджений production-код: commit `2ad3e01837b1d56dbcd07a391b9a867a9afdc365`; інтеграційна частина SignalSynch — commit `1027f1737b63561dbaf28ce54a6965dee473c7b3`. `laba-portal.service`, `koro-signal-sheets-sync.service` і Caddy активні, `/healthz` відповідає, `PRAGMA quick_check` повертає `ok`, а два послідовні штатні цикли передали 221 рядок без pending-дій або помилок.
...
- Нативне приміткове поле робочого ідентифікатора з кожного рядка `Обліку` синхронізується в `source_comment` і показується в картці як `Коментар з Обліку` замість технічного поля `Джерело`. Поле доступне лише для читання й не змішується з редагованими `Примітками до роботи`.
