...
- Поточна версія застосунку: `0.23.3`.
- Останній підтверджений production-код: commit `5a816f2fd19e246e1063e5503c59a8d6c78fc73e`; інтеграційна частина SignalSynch — commit `8f81ae22bb2e428de89b608d26e6adf7aebe63f3`. `laba-portal.service`, `koro-signal-sheets-sync.service` і Caddy активні, `/healthz` відповідає, `PRAGMA quick_check` повертає `ok`, а production-тести LABA проходять `11/11`. Перед deployment створено SQLite backup `/opt/laba/backups/portal-20260907-1655-before-0.23.3.db`; попередню production-збірку збережено в `/opt/laba-previous-20260907T1655Z-0.23.2`. `.env`, робочу базу та Caddyfile під час deployment не змінено; контрольна сума Caddyfile залишилася незмінною. Стартова й наступна інтервальна синхронізації передали по 221 запису без помилок і незавершених outbox-дій.
