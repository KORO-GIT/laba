...
Raspberry Pi має зарезервовану адресу `192.168.0.63`. Vendor WayVNC слухає TLS/PAM `192.168.0.63:5900`; у локальній мережі до цієї адреси підключається звичайний VNC-клієнт. Окремий `laba-wayvnc-web.service` слухає `192.168.0.63:5901`, використовує PAM/VeNCrypt Plain і systemd IP policy, яка приймає лише VPS та власні адреси Pi. Plain потрібен через обмеження noVNC і знаходиться лише всередині HTTPS/WSS та Tailscale/WireGuard.
