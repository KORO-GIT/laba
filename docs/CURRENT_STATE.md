...
Актуально на 2026-09-06. Це безпечний handoff для продовження роботи з іншого ПК або в новій задачі Codex. Секретів у цьому файлі немає.

## Код і production

- Репозиторій: `https://github.com/KORO-GIT/laba`, гілка `main`.
- Поточна версія застосунку: `0.14.1`.
...
- Головна сторінка показує лише доступні користувачу модулі: `Майстерня`, `Сервіс`, `Пристрої`.
- `Майстерня` отримує з усіх налаштованих таблиць `Облік` борти зі статусами `ПОТРЕБУЄ ОГЛЯДУ` або `ТЕХНІЧНІ ПРОБЛЕМИ`. Колонки: `Нові`, `На огляді`, `У ремонті`, `Відкладено`, `Готово`.
- Переміщення картки до `Готово` створює outbox-дію для точного вихідного рядка. SignalSynch повторно перевіряє рядок і змінює статус на `НА ОБЛІТ`, після чого підтверджує дію LABA.
- `Сервіс` отримує борти зі статусом `ПОТРЕБУЄ СЕРВІСУ`. Колонки: `Нові`, `Готуються документи`, `Документи подано`, `Очікує відправлення`, `Відправлено`.
