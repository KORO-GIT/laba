...
- Поточна версія застосунку: `0.8.0`.
...
Raspberry Pi має зарезервовану адресу `192.168.0.63`. WayVNC слухає лише `192.168.0.63:5900`, використовує PAM і vendor detached-session service Raspberry Pi OS. У локальній мережі до цієї адреси можна підключатися звичайним VNC-клієнтом.

В адмінпанелі `/admin` є вкладка «Робочий стіл». Після натискання «Віддалене підключення» noVNC `1.7.0` відкриває exact same-origin WebSocket LABA. Шлях доступний лише адміністратору, має окремі ліміти та аудит. На VPS websockify слухає тільки `127.0.0.1:6080` і через Tailscale subnet route з’єднується лише з `192.168.0.63:5900`. Пароль Pi вводиться після RFB-запиту, у LABA не зберігається. Порти `5900` і `6080` не відкриті в Caddy/UFW.
