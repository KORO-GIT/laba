...
Актуально на 2026-09-11, після оновлення синхронізації Сервісу о 12:34:56 UTC (15:34:56 за Києвом). Це handoff для продовження роботи з іншого ПК або в новій задачі Codex. Секретів у цьому файлі немає.
...
- Борт із вхідним сервісним статусом і розташуванням **борта** `КИЇВ`/`НА РЕМОНТІ` тепер потрапляє у `Відправлено`; для `ТАРИ` перевіряється розташування **тари** `КИЇВ`. Підтверджені картки залишаються видимими, включно з відновленням відповідних раніше прихованих рядків. Зворотна зміна розташування повертає картку з фінальної колонки у налаштовану вхідну. Правила/конфлікти/обмеження — `SERVICE_SYNC.md`; нижчий опис приховування таких карток є історичним.
- Ручне відправлення зберігає попередні цілі запису в Облік, але не архівує сервісну картку після ack. Snapshot не породжує write-відлуння; старі pending-дії скасовуються при зміні джерела, пізній ack ігнорується. Проміжні колонки ручні, коментарі з таблиць read-only, Майстерня зберігає старий статусний workflow.
- Міграція додає nullable `source_board_location` / `source_case_location`, не змінює старі значення. Windows та чистий Linux staging: `npm ci`, check, **45/45 tests**, audit **0**. Дворазова міграція приватної копії production зберегла всі старі значення **42 таблиць**, quick_check/FK ok. Перший холодний Windows запуск одразу після npm ci мав startup timeouts; повторний повний suite та Linux suite пройшли.
- Browser regression тільки на synthetic localhost: таблиця → shipped → повернення, polling без reload, історія `Облік`, desktop1440/mobile390, без JS errors/overflow. IAB недоступний; використано локальний Playwright Chrome. Production-тестових записів і повідомлень не створювали.
- Backup **`/opt/laba/backups/service-sync-f1eac7b-20260911-123456/portal.db`**, попередній runtime `source.tar.gz`, каталог root700/файли root600. Поточну БД не підміняли, inode збережений; ERP-таблиці та notes/report_number після restart збіглися з backup. Залежності production не перевстановлювали: lock незмінний, clean install пройшов у `/opt/laba-stage-f1eac7b`.
- Start **2026-09-11 12:34:56 UTC**, NRestarts0, health200; без JWT Service/ERP API401, SQLite/FK/write-path ok, root750/env640/data700/DB660. `.env`, Caddy hash та час запуску всіх сусідніх служб незмінні. SignalSynch працює без змін коду/конфігурації; жодного ручного запису таблиць не запускали.
- Штатний snapshot **12:35:57 UTC** підтвердив live-роботу: 167 активних сервісних карток, з них **30 у shipped**, 137 у new; 29 переміщень старих карток записані з автором `Облік`, нова картка створена відразу у фінальній колонці. 10 старих неактуальних сервісних рядків лишилися прихованими; IDs/рапорти/нотатки не стиралися. Pending/failed outbox відсутні, попередні 2 applied незмінні: write-відлуння немає. Інтервал SignalSynch перевірений як60000ms; UI polling8s (2.5s за pending). Журнал LABA рівня err порожній.
