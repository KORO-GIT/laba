...
Для 0.27.0 використано перевірений **code-only archive з оновленням на місці `/opt/laba`**, без заміни всього каталогу. До запису звірити список/хеші поточного коду з останнім release і новий archive; виключити `.env`, `.git`, `node_modules`, data, backups, сертифікати й секрети. Підготувати приватний archive фактичного попереднього коду. Зупинити тільки LABA, створити свіжий SQLite backup, перевірити його integrity, оновити тільки код, виконати `npm ci --omit=dev`, зберегти root 750 / env 640 / data 700 / DB 660, перевірити Sharp під `laba`, запустити й перевірити health. Не використовувати broad rename/delete, не копіювати стару БД назад і не змінювати інші сервіси. Після release звірити всі code hashes, старі ERP-рядки з backup, auth/401 (також приватні зображення), журнали, права, Caddy hash та стан сусідніх служб. Тестову публікацію/завантаження виконувати тільки на synthetic localhost, не в production.

Версія `0.25.0` додає робочі команди, `erp_crews_v1`, три нові таблиці, nullable `crew_id` та `work_round` до tasks/time entries, `work_mode=individual` до tasks. Старі дані не переписуються. Нові `.env`, Caddy, порти, залежності або Pi agents не потрібні. Див. `ERP_TEAMS.md`. Посилений `scripts/erp-migration-check.mjs` перевіряє всі наявні таблиці, зокрема ERP: незмінність старих колонок/рядків, markers, quick_check/FK після двох міграцій приватної тимчасової backup. Обов'язкові 25 tests та обидва localhost browser-check сценарії перед release.

**Rollback 0.25.0:** стара ERP `0.24.0` не розуміє спільних таймерів і команд. Після реальної командної роботи не повертати старий ERP у режим запису без окремого контрольованого блокування ERP та перевірки семантичної сумісності. Переважно fix-forward. Актуальну БД завжди зберігати; старим backup нові операції не стирати.

Версія `0.24.0` додає окремий `/erp`, API та таблиці `erp_*` з індексами/append-only triggers і записом `erp_v1` у `schema_migrations`. Міграція автоматична, ідемпотентна й не змінює наявні рядки модулів LABA. ERP не потребує нових `.env`, публічних портів, Caddy-правил або SignalSynch/Pi agents. Глобальні адміністратори отримують доступ автоматично; іншим користувачам ERP-роль призначається окремо. Demo seed **ніколи** не запускати на production.

Перед першим ERP release: точні `npm ci`, `npm run check`, `npm test`, `npm audit --omit=dev` локально й на staging-копії release archive. `.npmrc` містить `ignore-scripts=true`, тому native SQLite runtime обов'язково перевіряється тестами на цільовій ОС. На копії production SQLite перевірити `.backup`, `quick_check`, `foreign_key_check` і незмінність усіх старих таблиць, крім доданого migration marker. Збережені дані не друкувати.

Збирати archive з конкретного Git commit (`git archive`), не з довільного dirty worktree. Перед заміною ще раз fetch/порівняти origin/main і фактичний код VPS: зміни з іншого ПК не перезаписувати. Staging має залишатися окремим від `/opt/laba`; перед atomic switch зупинити тільки `laba-portal`, скопіювати актуальні `.env`, `data/`, `backups/`, перевірити їх збереження. Кореневий каталог `root:laba 0750`, `.env root:laba 0640`, `data/` і `backups/` — `laba:laba 0700`. Не виконувати наведений вище `chmod 0600 .env` для root-owned production-конфігурації: користувач сервісу повинен мати доступ через групу.

Після релізу: health, quick_check, read-only перевірка захисту ERP без JWT, код/статичні assets та незмінність Caddy/сусідніх units. Точний функціональний commit, backup, попередній каталог і результати записати в `CURRENT_STATE.md`, а незавершені етапи — в `ERP_CONTINUATION.md`; push обох документів. Для rollback зберігати **актуальну БД** та перевіряти сумісність попереднього коду: до-ERP `0.23.3` ігнорує `erp_*`, але ERP `0.24.0` не сумісна зі спільними таймерами `0.25.0`. Не відновлювати стару БД поверх нових операцій без окремого рішення про втрату даних.
...
Версія `0.15.0` додає таблиці `workflow_boards`, `workflow_lanes` і `workflow_source_statuses`. Під час першого запуску вони заповнюються поточними назвами, колонками та правилами `Майстерні` й `Сервісу`; наявні картки, дозволи та outbox-дії не перебудовуються. Наступні запуски не повертають видалені адміністратором статуси або змінені назви до стандартних значень.

Версія `0.16.0` додає таблиці `workflow_card_statuses` і `maintenance_card_status_assignments` для налаштовуваних кольорових статусів карток. Міграція не призначає статуси автоматично й не змінює колонки, примітки, права або accounting outbox. Одна картка може мати кілька статусів; видалення визначення каскадно прибирає тільки відповідні призначення.

Версія `0.17.0` додає окремі таблиці `workflow_card_labels` і `maintenance_card_label_assignments` для кольорових міток складності або типу ремонту. Мітки не замінюють статуси роботи, не призначаються на наявні картки автоматично й не впливають на колонки або accounting outbox. Одна картка може мати кілька міток; видалення мітки каскадно прибирає лише її призначення.

Версія `0.18.0` додає `schema_migrations`, колонку `maintenance_cards.report_number` та одноразово призначає статус `ВТРАЧЕНИЙ` дошці `Сервіс`. Під час синхронізації такі записи показуються як засіб `ТАРА`, але вихідні координати рядка, номер борта та ідентифікатори не змінюються. Міграція не заповнює номери рапортів і не змінює наявні картки інших статусів.

Версія `0.20.0` додає колонку `maintenance_cards.source_comment`. Вона заповнюється тільки внутрішньою синхронізацією з нативного приміткового поля відповідного рядка `Обліку`, показується в картці замість технічного поля `Джерело` і не входить до редагованих полів LABA. Міграція створює колонку з порожнім значенням, тому наявні робочі примітки в `maintenance_cards.notes` не змінюються.

Версія `0.22.0` не змінює схему БД. Вона відкриває налаштування конкретної дошки користувачеві з модульним рівнем `Адміністратор`, не надаючи доступу до глобальних користувачів, пристроїв та інших адміністративних API. Для карток `ТАРА` додається фільтр за вихідним засобом. Поле `source_comment` тепер отримує значення з окремої колонки `Коментар` таблиці `Облік`, тому разом із LABA потрібно оновити SignalSynch до відповідної версії інтеграції.

Версія `0.23.2` не змінює схему БД або серверну бізнес-логіку. Вона оновлює спільну дизайн-систему всіх сторінок: текстові стрілки замінено стабільними SVG-іконками, `select` отримали однаковий центрований шеврон, мінімальні розміри робочого тексту й елементів керування збільшено, а дошки, картки, форми, таблиці та мобільні компонування перевірено на desktop і viewport `390×844`.

Версія `0.23.3` не змінює схему БД або серверну бізнес-логіку. Вона прибирає повторну entrance-анімацію колонок під час кожного render після drop, вимикає transform-transition і нахил drag-preview, пропускає тихе перемальовування незміненої дошки та скасовує відкладений touch-drag після короткого натискання.

Версія `0.21.0` розширює `accounting_outbox` типом дії, ключем вихідної колонки та окремими цілями розташування борта й тари. Для `Сервіс → Відправлено` статус рядка не змінюється: `ВТРАЧЕНИЙ` отримує тільки розташування тари `КИЇВ`, а `ПОТРЕБУЄ СЕРВІСУ` — розташування борта й тари `НА РЕМОНТІ`. Поточні розташування надходять у вхідному snapshot, тому підтверджені картки не створюються повторно, доки їхній стан в `Обліку` відповідає завершеній сервісній операції. Старі status-дії мігруються без зміни цільового статусу.

Версія `0.21.1` не змінює схему БД. На картках канбану прибрано дубльований вхідний статус `Обліку`, а текст налаштовуваних статусів картки збільшено для кращої читабельності.

Версія `0.21.2` не змінює схему БД. Втрачений борт не створюється в `Сервіс` як `ТАРА`, якщо поточне розташування тари в `Обліку` вже дорівнює `КИЇВ`; порівняння нормалізує регістр і зайві пробіли. Наявна картка також прибирається після отримання такого стану в наступному повному snapshot.
...
**Виняток для ERP:** наведена нижче загальна процедура не дозволяє запускати старий ERP writable поверх нової семантики даних. Для `0.25.0` shared-таймерів див. `ERP_TEAMS.md`, для `0.26.0` кількостей `quantity_scale=1000` — `ERP_MATERIALS.md`. Пріоритет fix-forward; заборонено замінювати БД старим backup поверх нових записів користувачів. Аварійне блокування ERP/звірка даних потребують окремого плану.
