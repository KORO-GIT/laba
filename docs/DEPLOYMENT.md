...
Після релізу: health, quick_check, read-only перевірка захисту ERP без JWT, код/статичні assets та незмінність Caddy/сусідніх units. Точний функціональний commit, backup, попередній каталог і результати записати в `CURRENT_STATE.md`, а незавершені етапи — в `ERP_CONTINUATION.md`; push обох документів. Для rollback спочатку повернути попередній **код зі збереженням актуальної БД**: стара версія ігнорує `erp_*`. Не відновлювати стару БД поверх нових операцій без окремого рішення про втрату даних.
