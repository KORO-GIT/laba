...
- LABA не звертається до Google напряму. Вона формує idempotent outbox-дії з точними `spreadsheetId`, `sheetId` і номером рядка; SignalSynch перед записом повторно перевіряє джерело, борт і поточний статус рядка. Відсутній у частковому snapshot рядок не видаляється масово.
