...
- Двосторонній `Сервіс` зберігає останні розташування джерела й не створює запису-відлуння при імпорті підтвердженого відправлення. Зміна джерела скасовує застарілі pending-дії, пізній ack скасованої дії ігнорується. Це не атомарна транзакція з Google: правка таблиці безпосередньо під час віддаленого запису лишається ризиком. Правила — `SERVICE_SYNC.md`.
...
### ERP: прикладний захист і цілісність

- `/erp` та `/api/erp/*` вимагають активного користувача LABA й окрему ERP-роль. Права перевіряються на сервері при кожному запиті; майстер отримує власну чергу/зміни та роботи своїх активних команд. Склад команди містить імена, але не e-mail колег. Роль ERP або старшого команди не відкриває пристрої, існуючі дошки чи глобальну адмінку.
- Команди `0.25.0`: членство/старший/версія перевіряються серверно. Захват операції партії, особистий таймер, аудит та UUID-результат атомарні. Один майстер має один активний таймер; пауза його зміни не зупиняє інших. Старший/керівник завершує спільну операцію тільки після завершення всіх поточних внесків. Перепризначення/доробка зберігають старий час та історичних виконавців для незалежного QC. Деталі — `ERP_TEAMS.md`.
- Команди вимагають exact `Origin` (із портом у development), `X-Portal-Request: 1` і UUID `X-Erp-Request-Id`; Fetch Metadata відхиляє cross-site/same-site запити. Строгі Zod-схеми, body limit 256 KiB та окремі rate limits обмежують ввід.
- Дані записуються параметризованими SQL-запитами в `BEGIN IMMEDIATE`: дія, аудит і результат ідемпотентності атомарні. Повторна доставка того самого запиту не дублює прийомку/списання; інший payload із тим самим UUID відхиляється. Версії записів запобігають непомітному перезаписуванню з іншого телефона/ПК.
- Складський журнал та ERP-аудит append-only, включно з SQLite triggers. Не допускаються від'ємний залишок, використання власності іншого клієнта, подвійна видача, видача без QC або з неврахованим залишком на робочому місці. Контролер не може прийняти виріб, над яким працював сам, включно з попередніми циклами доробки.
- Користувацькі рядки потрапляють у DOM через `textContent`, не HTML. ERP не приймає shell, довільні зовнішні URL чи CDN. CSP/anti-indexing успадковані від порталу. Єдиний ERP upload0.27.0 — фото адміністратора до інструкції: сигнатура+decode JPG/PNG/WebP, перекодування/видалення metadata, приватний BLOB, ліміти обсягу/пікселів/паралельності, квоти та ACL. Тільки цей endpoint має body limit8MiB+1024 замість256KiB. Чернетки/архівні та непублічні фото недоступні майстрам. Деталі й залишкові ризики — `ERP_GUIDES.md`.
- Телефон не зберігає робочі дані в localStorage/IndexedDB і не має offline-черги. Зберігається тільки вибір теми `laba.theme`. Дія вважається виконаною після підтвердження сервера. Втрата мережі не повинна інтерпретуватися як успішне списання.
- Час роботи є явним обліком операцій, а не доказом фізичної присутності. Відкриті зміни обмежені 16 годинами; керівник може закрити забуту зміну з причиною й аудитом. GPS, мікрофон і приховане спостереження не використовуються.
- Це не незалежний penetration test. Компрометація адміністратора/root, викрадення активної сесії чи злам особистого телефона залишаються ризиками. Для робочих телефонів потрібні блокування екрана, оновлення ОС і вихід з акаунта при втраті; MFA для підтримуваного IdP та регулярне відкликання доступу звільнених працівників.
- Git містить код і міграції, але не робочу БД. Копія backup на тому самому VPS не захищає від втрати VPS; окреме зашифроване offsite-сховище й перевірка відновлення ще потребують налаштування.
