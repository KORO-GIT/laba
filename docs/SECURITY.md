...
- Адміністративні зміни, включно з конфігурацією workflow, кастомними статусами, кольоровими мітками карток і правами окремих дошок, потребують глобальної ролі `admin`, same-origin і спеціального заголовка, обмежені rate limit та записуються до аудиту. Головний адміністратор примусово зберігає доступ `admin` до кожної дошки. API картки приймає лише ідентифікатори статусів і міток, що належать її дошці.
