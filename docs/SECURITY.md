...
- `Майстерня`, `Сервіс` і `Пристрої` мають незалежні server-side дозволи `viewer`, `operator`, `admin`; приховування елементів у браузері не використовується як засіб авторизації.
- Bootstrap-адміністратор примусово зберігає глобальну роль `admin`, активний стан і рівень `admin` у кожному модулі. API відхиляє його перейменування, вимкнення або пониження.
- Обмін із Google `Обліком` проходить лише через внутрішній endpoint на loopback VPS та окремий `ACCOUNTING_SYNC_TOKEN` щонайменше 32 символи. Значення токена порівнюється без залежного від даних часу й не повертається клієнту.
- LABA не звертається до Google напряму. Вона формує idempotent outbox-дії з точними `spreadsheetId`, `sheetId` і номером рядка; SignalSynch перед записом повторно перевіряє джерело, борт і поточний статус рядка. Новий статус або розташування приймаються лише з канонічних варіантів шаблону `Обліку`, а зміна кількох розташувань виконується одним batch-запитом. Відсутній у частковому snapshot рядок не видаляється масово.
- Для втрачених бортів назва засобу `ТАРА` є лише нормалізованим представленням у модулі `Сервіс`: точні координати джерела, номер борта й ідентифікатори залишаються незмінними. Номер рапорта обмежений 120 символами та редагується лише користувачем із правами виконавця або адміністратора цієї дошки.
...
- Конфігурація workflow, кастомні статуси, кольорові мітки та права учасників конкретної дошки доступні глобальному адміністратору або користувачеві з модульним рівнем `admin` саме для цієї дошки. Інші адміністративні API потребують глобальної ролі `admin`. Усі такі зміни вимагають same-origin і спеціальний заголовок, обмежені rate limit та записуються до аудиту. Головний адміністратор примусово зберігає доступ `admin` до кожної дошки. API картки приймає лише ідентифікатори статусів і міток, що належать її дошці.
