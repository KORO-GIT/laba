...
- Детектор хлопків читає PCM тільки локально через exact PipeWire node Logitech C270. Аудіо не записується на диск, не передається у LABA або мережу й обробляється блоками по 20 мс; єдина можлива дія — allowlisted MPRIS `play-pause`.
