...
Робочі команди `0.25.0` описані в `docs/ERP_TEAMS.md`; прочитати перед змінами призначень, часу, змін, QC або ролей. Поточний checkpoint готується у `codex/erp-teams` від `1991d0e`; production поки залишається `0.24.0`, доки `CURRENT_STATE.md` не зафіксує перевірений release. Не робити семантично несумісний rollback старої ERP поверх спільних таймерів.
