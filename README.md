...
- канбан `Сервіс` для бортів зі статусом `ПОТРЕБУЄ СЕРВІСУ` та втрачених бортів (`ТАРА`) з номером рапорта й двосторонньою синхронізацією відправлення з `Обліком`: борт у `КИЇВ`/`НА РЕМОНТІ` або тара в `КИЇВ` показуються у `Відправлено`; ручне відправлення пише розташування в таблицю, а картка залишається на дошці. Правила й конфлікти — [docs/SERVICE_SYNC.md](docs/SERVICE_SYNC.md);
