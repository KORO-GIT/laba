...
Cloudflare Access подтверждает личность и защищает форму входа. Админка LABA является главным списком доступа: она определяет, разрешён ли подтверждённый e-mail, его роль и список устройств.

## Добавление пользователя

1. Администратор открывает `/admin`, добавляет точный e-mail, выбирает роль и назначает устройства.
2. Пользователь открывает `https://laba.zpseapil.club`, вводит тот же e-mail и получает одноразовый код от Cloudflare.
3. После проверки кода LABA повторно сверяет e-mail с локальным allowlist. Пользователь, которого нет в админке или который отключён, доступа не получает.

Для добавления обычного пользователя изменять Cloudflare вручную не нужно. Участники Cloudflare-аккаунта также могут использовать существующий вход Cloudflare. Завершить сессию можно через `/cdn-cgi/access/logout`.
