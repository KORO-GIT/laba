...
- ERP «Інструкції»: закрита бібліотека для майстрів, покрокові матеріали з фото, чернетки/публікація/архів лише адміністраторами, пошук і мобільне читання. Інструкція — [docs/ERP_GUIDES.md](docs/ERP_GUIDES.md);
- стартовий вибір модулів `Виробництво ERP`, `Майстерня`, `Сервіс` і `Пристрої` відповідно до прав користувача;
- ERP `/erp`: клієнти, партійна прийомка, заводські/внутрішні номери, шаблони операцій, призначення майстрам, зміни й час, незалежний контроль якості, часткова видача та склад із розділенням власності;
- ERP робочі команди: спільна черга партії або одночасна робота кількох людей над складним виробом; старший, особисті внески/таймери та збережена історія. Інструкція — [docs/ERP_TEAMS.md](docs/ERP_TEAMS.md);
- ERP матеріали: норми витрат на тип ремонту/модернізації, потреба під партії, власний і клієнтський запас, внутрішні попередження та заявки з частковою прийомкою. План не списує склад: фактичну витрату підтверджують окремо. Інструкція — [docs/ERP_MATERIALS.md](docs/ERP_MATERIALS.md);
- мобільне робоче місце майстра: власна черга з пошуком і пагінацією, початок/пауза/завершення операції; графітово-помаранчева LABA та перемикач світлої теми;
- канбан `Майстерня` для бортів зі статусами `ПОТРЕБУЄ ОГЛЯДУ` та `ТЕХНІЧНІ ПРОБЛЕМИ` в Google-таблицях `Облік`;
- канбан `Сервіс` для бортів зі статусом `ПОТРЕБУЄ СЕРВІСУ` та втрачених бортів: втрачені записи створюються як окремий засіб `ТАРА`, проходять ті самі етапи підготовки документів і мають поле `Номер рапорта`; `ТАРА`, яка вже має розташування тари `КИЇВ`, у сервісі не створюється; у фінальній колонці `Відправлено` для `ТАРА` розташування тари стає `КИЇВ`, а для сервісного борта розташування борта й тари стають `НА РЕМОНТІ`;
- автоматичне повернення статусу `НА ОБЛІТ` у точний рядок `Обліку` після завершення ремонту та автоматичне прибирання підтвердженої картки з відкритої дошки без перезавантаження сторінки;
- одностороння синхронізація тексту з колонки `Коментар` відповідного рядка `Обліку` в картку ремонту без можливості змінити цей текст у LABA;
- редактор канбан-дошок: назви, кольори й порядок колонок, проміжні етапи, вхідні статуси `Обліку`, кінцеві статуси, кастомні статуси карток, кольорові мітки та права користувачів;
- кілька одночасних статусів роботи на картці, налаштованих окремо для кожної дошки, наприклад `Чекаємо запчастини`;
- окремі кольорові мітки для рівнів складності або типів ремонту: кілька міток на картці, компактні кольорові смуги та легенда над дошкою;
- стабільне перетягування карток без повторної анімації колонок після drop; короткий touch не залишає drag-preview поверх відкритої картки;
- незалежні рівні доступу `Без доступу`, `Перегляд`, `Виконавець` і `Адміністратор` для кожного модуля;
...
Доступ до кожного модуля призначається незалежно. Рівень `Перегляд` дозволяє читати дошку, `Виконавець` також змінює етапи й примітки, а `Адміністратор` має повне керування модулем. Адміністратор окремої дошки бачить на ній кнопку `Налаштування` й може керувати тільки її колонками, статусами, мітками та учасниками; користувачі, пристрої й інші системні розділи залишаються закритими. Глобальний адміністратор налаштовує всі дошки та їхні доступи у `/admin` → `Дошки`. Головний bootstrap-адміністратор завжди має рівень `Адміністратор` у всіх модулях і не може бути вимкнений або понижений через UI/API.

Для модуля `Пристрої` додатково діють індивідуальні призначення конкретних пристроїв:
...
Потрібен Node.js 22+; для перевірок також Python 3. `.npmrc` вимикає install hooks; поточний SQLite-пакет містить готові native binaries.
...
ERP є першим робочим контуром, а не завершеною бухгалтерською ERP: зарплата, закупівлі/BOM, резервування, QR/фото й друк актів залишаються наступними етапами. Нові акаунти майстрів створюються у `/erp` → `Команда` без доступу до інших модулів. Наявним користувачам треба окремо призначити ERP-роль. Глобальний адміністратор має ERP-доступ автоматично. Детальна специфікація й обмеження — [docs/ERP.md](docs/ERP.md); відтворюваний запуск і продовження з іншого ПК — [docs/ERP_CONTINUATION.md](docs/ERP_CONTINUATION.md).
