...
Для доступу ззовні адміністратор відкриває `/admin` → «Робочий стіл» і натискає «Віддалене підключення». Браузерний noVNC з’єднується з exact WebSocket-шляхом LABA; сервер повторно перевіряє роль `admin` і same-origin, прибирає cookies/authorization, після чого передає VNC-трафік до `websockify` на loopback VPS. `websockify` звертається до окремого PAM endpoint `192.168.0.63:5901` через Tailscale subnet route. Окремий attach-agent без мережевого доступу прив’язує цей WayVNC до активного Wayland compositor та автоматично відновлює прив’язку після перезапуску графічної сесії. Systemd IP policy на Pi приймає endpoint лише від VPS/власного вузла Pi; інші LAN-клієнти використовують TLS endpoint `5900`. Публічні порти `5900`, `5901` і `6080` не відкриваються.
