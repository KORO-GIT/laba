...
- канбан `Сервіс` для бортів зі статусом `ПОТРЕБУЄ СЕРВІСУ` та втрачених бортів: втрачені записи створюються як окремий засіб `ТАРА`, проходять ті самі етапи підготовки документів і мають поле `Номер рапорта`;
