...
- одностороння синхронізація накопичених коментарів із нативного приміткового поля рядка `Обліку` в картку ремонту без можливості змінити цей текст у LABA;
...
Доступ до кожного модуля призначається незалежно. Рівень `Перегляд` дозволяє читати дошку, `Виконавець` також змінює етапи й примітки, а `Адміністратор` має повне керування модулем. Глобальний адміністратор налаштовує кожну дошку та її доступи у `/admin` → `Дошки`. Головний bootstrap-адміністратор завжди має рівень `Адміністратор` у всіх модулях і не може бути вимкнений або понижений через UI/API.
