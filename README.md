...
Доступ до кожного модуля призначається незалежно. Рівень `Перегляд` дозволяє читати дошку, `Виконавець` також змінює етапи й примітки, а `Адміністратор` має повне керування модулем. Головний bootstrap-адміністратор завжди має рівень `Адміністратор` у всіх модулях і не може бути вимкнений або понижений через UI/API.
