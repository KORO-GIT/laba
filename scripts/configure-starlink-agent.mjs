...
if (token.length < 32 || !/^[A-Za-z0-9_-]+$/.test(token)) throw new Error('Starlink agent token is invalid');

const envPath = '/opt/laba/.env';
const current = fs.readFileSync(envPath, 'utf8');
const values = new Map([
  ['STARLINK_AGENT_URL', 'http://100.69.168.10:1986'],
  ['STARLINK_AGENT_TOKEN', token]
]);
const seen = new Set();
...
console.log('Configured STARLINK_AGENT_URL and STARLINK_AGENT_TOKEN in /opt/laba/.env');
